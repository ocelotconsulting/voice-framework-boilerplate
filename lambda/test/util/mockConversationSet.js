const conversationSet = {
  engagement: {
    craftResponse: ({ finalWords }) => {
      if ( finalWords ) {
        return '';
      }
      return "engagement text";
    },
    acceptIntent: async ({ conversationStack, currentSubConversation, sessionAttributes, intent, topConversation, poppedConversation, fallThrough }) => {
      if (!fallThrough) {
        if (topConversation) {
